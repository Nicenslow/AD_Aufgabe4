...
public class KuerzesterPfadTiefensuche implements KuerzesterPfad {
	 ...
	 *
	 * @param graph
	 *            Graph mit Knoten und gewichteten Kanten
	 * @param startKnoten
	 *            Startknoten
	 * @param zielKnoten
	 *            Zielknoten
	...
	public List<Knoten<Zelle>> berechneKuerzestenPfad(Graph<Zelle> graph, Knoten<Zelle> startKnoten,
		...
		Comparator<Knoten<Zelle>> comparator = Comparator
				.comparingDouble(k -> k.getElement().getEntfernung(zielKnoten.getElement()));
		...
		nachbarn.sort(comparator.reversed()); // nach dem Entfernen sortieren
		stack.addAll(nachbarn);

		while (!stack.isEmpty() && nachbarEnthalten(nachbarn, startKnoten)) { // Pfad
																				// rekursiv
																				// durchlaufen
			List<Knoten<Zelle>> temp = berechneKuerzestenPfad(graph, stack.pop(), zielKnoten);
			if (temp != null) {
				if (temp.size() > 1) { // Rekursion erfolgreich, wenn mehr als
										// ein Element vorhanden!!
	...
	private List<Knoten<Zelle>> entferneBenutzte(List<Knoten<Zelle>> input) {
		...
		for (Knoten<Zelle> element : input) { // prüfen ob Element bereits
												// vorhanden
	...
	private boolean nachbarEnthalten(List<Knoten<Zelle>> liste, Knoten<Zelle> start) {
		if (liste.contains(stack.peek()) && stack.peek() != start) {
			return true; // also wenn Nachbar in Liste vorhanden, dann true
							// returnen
		}
		return false;
	}
